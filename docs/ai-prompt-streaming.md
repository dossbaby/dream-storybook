...
#### 3. ✅ Hidden Insight (차원의 틈) 대폭 개선 완료 (2025-12-12)
...
- ✅ 시작 방식 다양화: '원래 안 말할라 그랬는데' 금지
- ✅ 7가지 자연스러운 시작 문구 예시 추가:
  - '아 근데 이건 진짜..', '어 잠깐, 이거 봐봐요', '마지막으로 하나만 더'
  - '아 그리고 이건 좀 신기한데', '근데 솔직히 이게 진짜야'
  - '아 맞다 이것도..', '어? 이거 진짜 신기하네'
- ✅ MBTI/별자리 기반 반전 구조 추가 (#16 통합):
  - 이 사람의 성향이 왜 이 결과를 만들었는지 연결
  - MBTI/별자리 직접 언급 없이 특징만 자연스럽게 녹임

**수정 파일**: `src/hooks/useReading.js` (꿈/타로/사주 hiddenInsight 프롬프트 3곳)
...
**수정 파일**: `src/hooks/useReading.js` (꿈/타로/사주 3곳 수정)
...
#### 15. ✅ 내 리딩 전용 페이지 완료 (2025-12-12)

**변경사항**:
- `MyReadingsView.jsx` 신규 생성 - 내 리딩 전용 페이지
- BottomNav "내 리딩" 버튼: `feed` → `my-readings`로 변경
- `MyPage.jsx`에서 리딩 목록 섹션 완전 제거 (프로필/계정 섹션만 유지)
- FeedView 스타일 참고한 compact 카드 UI
- 탭: 타로 / 꿈 / 사주

**파일**:
- `src/components/my/MyReadingsView.jsx` (신규)
- `src/components/layout/BottomNav.jsx` (수정)
- `src/App.jsx` (라우팅 추가)
- `src/components/my/MyPage.jsx` (리딩 섹션 제거)
- `src/styles/views/mypage.css` (스타일 추가)

---

#### 16. ✅ 리딩 액션 모달 완료 (2025-12-12)

**기능**:
- 공개 설정: 전체 공개 / 링크 공유 / 비공개 3단계
- 링크 공유 선택 시 자동 URL 복사 + 토스트 메시지 ("링크가 복사 되었어요")
- 삭제하기 옵션 (인라인 확인 UI)
- 저장 버튼으로 명시적 저장 (자동 저장 X)

**UI 특징**:
- 모달 중앙 정렬 (모바일/데스크톱 모두)
- 토스트: 링크 공유 버튼 위에 overlay로 표시
- 어두운 배경 (`rgba(15, 15, 20, 0.95)`) + 보라색 테두리

**파일**:
- `src/components/my/MyReadingsView.jsx` (ReadingActionModal 컴포넌트)
- `src/styles/views/mypage.css` (모달 스타일)

---

#### 17. ✅ 프로필 유도 모달 완료 (2025-12-12)

**기능**:
- 타로 시작 시 프로필 미완성 상태면 모달 표시
- "맞춤 리딩을 받아보세요" 안내
- 혜택 목록: 정확한 해석, 사주 기반 분석, 별자리 운세
- "프로필 설정하기" / "나중에" 버튼

**파일**:
- `src/components/modals/ProfilePromptModal.jsx` (신규)
- `src/styles/components/modal.css` (스타일 추가)

---

#### 18. ✅ 터치 디바이스 여백 버그 수정 (2025-12-12)

**문제**: `@media (pointer: coarse)` CSS로 인해 `.more-btn`에 원치 않는 min-height/padding 적용
**해결**: `.more-btn`에 `!important` 오버라이드 추가
...
### 20. ✅ Hidden Insight MBTI/별자리 트위스트 완료 (2025-12-12)
...
- **수정 파일**: `src/hooks/useReading.js`

---

### 21. ✅ 질문 추천 기능 완료 (2025-12-12)

**배경**: 직접 ChatGPT/Claude에서 타로 리딩할 때 "재밌는 연애 질문 10개 줘" 식으로 영감 받음

**✅ 구현 완료**:
- "✨ 질문 추천 받기" 링크 클릭 → 모달 오픈
- 6개 카테고리: 💕연애, 💼직장, 💰재물, 🤔결정, 👥관계, 🔮미래
- Haiku API (claude-3-5-haiku-latest) 호출 → 7개 도파민 자극 질문 생성
- 시간 컨텍스트 포함 ("이번 주", "이번 달", "최근", "올해", "언제" 등)
- 클릭하면 모달 닫히고 질문 input에 자동 입력

**왜 API 호출?**:
- 하드코딩 = 반복되어 지루함
- 실시간 생성 = organic & random 느낌
- 카테고리별 맞춤 (연애/직장/재물/결정/관계/미래)

**신규 파일**:
- `src/components/tarot/QuestionSuggestionModal.jsx`
- `src/components/tarot/QuestionSuggestionModal.css`

**수정 파일**:
- `src/components/tarot/TarotInput.jsx` - 모달 연동
- `src/styles/views/tarot.css` - .suggestion-link 스타일
...
| ~~3~~ | ~~질문 추천 기능~~ | ~~⭐⭐~~ | ~~높음~~ | ✅ 완료 (2025-12-12) |
| ~~4~~ | ~~Hidden Insight MBTI~~ | ~~⭐⭐⭐~~ | ~~높음~~ | ✅ 완료 (2025-12-12) |
| ~~5~~ | ~~내 리딩 전용 페이지~~ | ~~⭐⭐~~ | ~~높음~~ | ✅ 완료 (2025-12-12) |
| ~~6~~ | ~~리딩 공개 설정 모달~~ | ~~⭐⭐~~ | ~~중~~ | ✅ 완료 (2025-12-12) |
| ~~7~~ | ~~프로필 유도 모달~~ | ~~⭐~~ | ~~중~~ | ✅ 완료 (2025-12-12) |
| 8 | 이미지 스타일 확장 | ⭐⭐⭐ | 중 | Freemium 수익화 |
| 9 | MBTI 탭 | ⭐⭐⭐⭐ | 높음 | 커뮤니티 확장 |
| 10 | 게이미피케이션 | ⭐⭐⭐⭐ | 매우 높음 | 장기 전략 |
| 11 | 실시간 채팅 | ⭐⭐⭐⭐⭐ | ? | MAU 확보 후 |
