...
> 최종 업데이트: 2025년 12월 13일 (v8 - 2K 이미지 업그레이드 + 최적화)
>
...
| 작업 | 상태 | 비고 |
|------|------|------|
| 브라우저 타이틀 변경 | ✅ 완료 | "점AI - 마음을 모를 때" |
| 스플래시 태그라인 추가 | ✅ 완료 | "마음을 모를 때" + 반짝이는 ✦ 데코 |
...
### Phase 26: 마이페이지 개편 & 프로필 기능 ✅ 부분 완료
> 내 리딩 전용 페이지 분리, 로그인/프로필 유도 UX 개선

#### Phase 26.1: 내 리딩 전용 페이지 ✅ 완료 (2025-12-12)
| 작업 | 상태 | 비고 |
|------|------|------|
| MyReadingsView.jsx 신규 생성 | ✅ 완료 | 내 리딩 전용 페이지 |
| BottomNav "내 리딩" 버튼 변경 | ✅ 완료 | `feed` → `my-readings` |
| App.jsx 라우팅 추가 | ✅ 완료 | `view === 'my-readings'` |
| MyPage.jsx 리딩 섹션 제거 | ✅ 완료 | 프로필/계정 섹션만 유지 |
| FeedView 스타일 compact 카드 UI | ✅ 완료 | 썸네일 + 질문/제목 + 날짜 + 태그 |
| 탭: 타로 / 꿈 / 사주 | ✅ 완료 | 타입별 필터링 |

#### Phase 26.2: 리딩 액션 모달 ✅ 완료 (2025-12-12)
| 작업 | 상태 | 비고 |
|------|------|------|
| ReadingActionModal 컴포넌트 | ✅ 완료 | MyReadingsView 내 인라인 컴포넌트 |
| 공개 설정: 전체/링크/비공개 | ✅ 완료 | 3단계 토글 |
| 링크 공유 시 자동 URL 복사 | ✅ 완료 | 토스트 메시지 표시 |
| 삭제하기 옵션 | ✅ 완료 | 인라인 확인 UI |
| 저장 버튼으로 명시적 저장 | ✅ 완료 | 자동 저장 X |

#### Phase 26.3: 프로필 유도 모달 ✅ 완료 (2025-12-12)
| 작업 | 상태 | 비고 |
|------|------|------|
| ProfilePromptModal.jsx 생성 | ✅ 완료 | 프로필 미완성 시 유도 |
| 조건: 프로필 완성도 < 50% | ✅ 완료 | name, birthDate 등 핵심 항목 체크 |
| "프로필 설정하기" / "나중에" 버튼 | ✅ 완료 | |
| 혜택 안내 | ✅ 완료 | 정확한 해석, 사주 분석, 별자리 운세 |

#### Phase 26.4: AI 프롬프트 개선 ✅ 완료 (2025-12-12)
| 작업 | 상태 | 비고 |
|------|------|------|
| 나이 언급 금지 강화 | ✅ 완료 | "38년을 살면서..." 우회 표현 금지 |
| Hidden Insight 다양화 | ✅ 완료 | '원래 안 말할라 그랬는데' 금지, 7가지 자연스러운 시작 |
| MBTI/별자리 트위스트 | ✅ 완료 | 직접 언급 없이 특징만 자연스럽게 녹임 |
| 구버전 ResultView 삭제 | ✅ 완료 | DreamResultView 신규, FortuneResultView 수정 |

#### Phase 26.5: 기타 UI 버그 수정 ✅ 완료 (2025-12-12)
| 작업 | 상태 | 비고 |
|------|------|------|
| 터치 디바이스 여백 버그 | ✅ 완료 | `.more-btn` `!important` 오버라이드 |
| 질문 추천 기능 | ✅ 완료 | QuestionSuggestionModal, Haiku API 호출 |

---

### Phase 27: 버그 & 개선 목록 (스트리밍 이후) ⏳ 대기
> `ai-prompt-streaming.md`에서 병합된 남은 작업들

#### Phase 27.1: UI/UX 버그 수정 ⏳ 대기
| # | 작업 | 상태 | 설명 |
|---|------|------|------|
| 1 | 분석 단계 스크롤바 강제 숨기기 | ⏳ 대기 | 모바일에서 스크롤바 노출 → `overflow: hidden` 또는 `::-webkit-scrollbar { display: none }` |
| 2 | Sticky Persona Card 위치 버그 | ⏳ 대기 | top nav 숨김 시 sticky 메뉴에 여백 발생 → `top` 값 동적 조정 |
| 4 | 카드 재선택 시 덱 상태 버그 | ⏳ 대기 | deselect 해도 덱에서 여전히 selected 표시 → 덱 선택 상태 리셋 |
| 9 | 프로필 설정 모달 레이아웃 | ⏳ 대기 | 생년월일/태어난 시간 7:3 → 50/50 split |
| 10 | 로고 클릭 동작 + 태그라인 | ⏳ 대기 | 클릭 시 메인페이지 새로고침, 태그라인 추가 |
| 13 | 리딩 이미지 Height 조정 | ⏳ 대기 | `.chapter-hero` 350px → 480px (16:9 비율) |
| 14 | 이미지 안 뜨는 버그 | ⏳ 대기 | Hero/Card 이미지 랜덤 누락 원인 파악 필요 |

#### Phase 27.2: 피드/커뮤니티 개선 ⏳ 대기
| # | 작업 | 상태 | 설명 |
|---|------|------|------|
| 7 | 피드/인기 리딩 CTA 추가 | ⏳ 대기 | "같은 질문에 참여하여 리딩을 받아보세요" 문구 |
| 11 | 피드 로딩 전략 & 검색 기능 | ⏳ 대기 | 무한 스크롤 페이지네이션, 검색 기능 |
| 12 | Desktop 레이아웃 기준 정립 | ⏳ 대기 | 1920px+ = 댓글창 오른쪽, 그 외 = 아래 |

#### Phase 27.3: 스트리밍 관련 ⏳ 대기
| # | 작업 | 상태 | 설명 |
|---|------|------|------|
| 5 | 도파민 메시지 시간 제한 체크 | ⏳ 대기 | 기존 ±50%~150% 시간 제한 로직 확인/제거 |

---

### Phase 28: 스트리밍 & Progressive Loading ⏳ 대기
> 2-3분 대기 → 체감 10초로 개선 (상세: `ai-prompt-streaming.md`)

#### ⚠️ 기존 Prompt 보존 원칙
> **기존에 설정된 prompt는 절대 수정하지 않는다.**
> `useReading.js`, `useImageGeneration.js`, `promptCache.js` 등에 있는 prompt들은 오랜 시간 공들여 작업한 것.
> 스트리밍 메커니즘만 변경, prompt 내용은 변경 금지.

#### Phase 28.1: 스트리밍 기본 구현 ⏳ 대기
| 작업 | 상태 | 설명 |
|------|------|------|
| `messages.create()` → `messages.stream()` 변경 | ⏳ 대기 | useReading.js |
| 스트리밍 버퍼 파싱 로직 구현 | ⏳ 대기 | 섹션별 완료 감지 |
| 콘솔 로깅 추가 | ⏳ 대기 | 진행 상황 추적 |

#### Phase 28.2: 이미지 병렬화 ⏳ 대기
| 작업 | 상태 | 설명 |
|------|------|------|
| 텍스트 섹션 완료 시 해당 이미지 즉시 시작 | ⏳ 대기 | hook 완료 → Hero 이미지 |
| 이미지 완료 콜백으로 상태 업데이트 | ⏳ 대기 | 섹션별 ready 상태 |

#### Phase 28.3: UI Progressive 업데이트 ⏳ 대기
| 작업 | 상태 | 설명 |
|------|------|------|
| 섹션별 ready 상태 관리 | ⏳ 대기 | readingState 객체 |
| 카드별 오픈 버튼 활성화 | ⏳ 대기 | canOpenCard1/2/3 |
| 결과 페이지 조기 전환 | ⏳ 대기 | hook 완료 시 전환 |

#### Phase 28.4: 도파민 Sonnet 통합 ⏳ 대기
| 작업 | 상태 | 설명 |
|------|------|------|
| Haiku 별도 호출 제거 | ⏳ 대기 | useDopamineMessages.js 수정 |
| Sonnet prompt에 dopamineMessages 출력 추가 | ⏳ 대기 | 기존 내용 수정 아님, 출력 형식 확장 |
| 분석 페이지에서만 도파민 표시 | ⏳ 대기 | hook 후 종료 |
...
### Phase 29: 미래 확장 아이디어 ⏳ 대기
> 우선순위별 기능 확장 로드맵

| 순위 | 기능 | 난이도 | 임팩트 | 상태 |
|------|------|--------|--------|------|
| 1 | 리딩 이미지 → 프로필 사진 설정 | ⭐⭐ | 중 | ✅ 완료 |
| 2 | 이미지 스타일 확장 (실사, 수채화) | ⭐⭐⭐ | 중 | ⏳ 대기 |
| 3 | MBTI 탭 & 커뮤니티 확장 | ⭐⭐⭐⭐ | 높음 | ⏳ 대기 |
| 4 | 게이미피케이션 (뱃지, 랭킹) | ⭐⭐⭐⭐ | 매우 높음 | ⏳ 대기 |
| 5 | 실시간 채팅 (24시간 펑) | ⭐⭐⭐⭐⭐ | ? | MAU 확보 후 |

---

### Phase 30: 2K 이미지 업그레이드 & 최적화 시스템 ✅ 완료
> Gemini 3 Pro 2K 해상도 + Firebase Resize Extension + WebP Fallback

| 작업 | 상태 | 비고 |
|------|------|------|
| **1. 이미지 해상도 업그레이드** | | |
| Gemini 이미지 생성 1K → 2K | ✅ 완료 | imageSize: '2K' (2752x1536) |
| Firebase Resize 사이즈 변경 | ✅ 완료 | 688x384, 1376x768, 2752x1536 |
| imageUtils.js 사이즈 상수 업데이트 | ✅ 완료 | FIREBASE_RESIZE_SIZES 변경 |
| **2. OptimizedImage 컴포넌트** | | |
| OptimizedImage.jsx 생성 | ✅ 완료 | WebP 리사이즈 시도 → 원본 fallback |
| FeedView 적용 | ✅ 완료 | 컴팩트/그리드 카드 모두 적용 |
| MyReadingsView 적용 | ✅ 완료 | 썸네일 + 프로필 크롭 영역 |
| TarotResultView 적용 | ✅ 완료 | 히어로/카드 이미지 |
| **3. 사이즈별 최적화** | | |
| 썸네일: medium (1376x768) | ✅ 완료 | 컴팩트 카드, 피드 썸네일 |
| 상세뷰: large (2752x1536) | ✅ 완료 | 그리드 카드, 리딩 결과 히어로 |
| 프로필: medium (1376x768) | ✅ 완료 | NavBar, MyPage 아바타 |
| **4. 프로필 사진 기능** | | |
| 프로필 사진 저장 버그 수정 | ✅ 완료 | useAuth에서 profilePhoto 병합 |
| 크롭 줌 최대치 500% 증가 | ✅ 완료 | max="3" → max="5" |

**기술 스택:**
- Gemini 3 Pro Image: 2K (2752x1536 for 16:9)
- Firebase Resize Extension: WebP 변환 + 3단계 리사이즈
- OptimizedImage: onError fallback 패턴

**이미지 사이즈 매핑:**
| 용도 | 사이즈 | 해상도 |
|------|--------|--------|
| small | 688x384 | 0.26MP |
| medium | 1376x768 | 1MP (1K) |
| large | 2752x1536 | 4.2MP (2K) |

**아이폰 Retina 대응:**
- 3x DPR (iPhone Pro): 1376px medium → 458pt 논리 픽셀 (충분)
- 데스크탑: 2752px large 사용
