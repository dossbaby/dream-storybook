...
#### Phase 24.2: 하단 네비게이션 개편 ✅ 부분 완료
> 홈(🏠) → 탐색(🔥) → 시작(✨) → 피드(📰) → 마이(👤)
> ↓
> 피드(🌀) → 인기(🔥) → 시작(모드별 이모지) → 리딩(📓) → 정보(💜)

| 작업 | 상태 | 비고 |
|------|------|------|
| 네비 라벨/이모지 변경 | ✅ 완료 | 피드(🌀)/인기(🔥)/리딩(📓)/정보(💜) |
| 시작 버튼 모드별 이모지 | ✅ 완료 | 타로🔮/꿈🌙/사주☀️ + 틸트 애니메이션 (Phase 25) |
| 피드 → 리딩 (내 기록 전용) | ⏳ 대기 | MyReadingsView 신규 생성 |
| 마이페이지에서 리딩 기록 제거 | ⏳ 대기 | 리딩 탭으로 이동 |
| 홈 → 커뮤니티 피드 (공개 질문들) | ⏳ 대기 | FeedView 수정 |
| 탐색 → 인기 (engagement 높은 순) | ⏳ 대기 | PopularView 신규 생성 |
| 마이 → 프로필만 | ⏳ 대기 | MyPage 간소화 |
